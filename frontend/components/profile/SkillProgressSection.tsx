      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-white/10">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-white tracking-tight">Missions & Curriculum Progress</h3>
          </div>
        ...
        {/* Filter Tabs */}
        <div className="flex flex-wrap items-center gap-1.5 bg-gray-900/90 p-1 rounded-xl border border-white/10 self-start sm:self-auto text-xs font-semibold">
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredMissions.map((mission, idx) => {
            ...
            return (
              <div
                key={`${mission.day}-${idx}`}
                className="p-3.5 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 transition-all flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-gray-300 font-mono text-xs font-bold shrink-0">
                    Day {mission.day}
                  </span>
                  <p className="text-xs font-semibold text-white truncate" title={mission.title}>
                    {mission.title}
                  </p>
                </div>
                ...
                <div className="shrink-0">
                  ...
                  {isRetry && (
                    <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 flex items-center gap-1">
                  ...
                  {isFailed && (
                    <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-red-500/10 text-red-400 border border-red-500/20 flex items-center gap-1">
