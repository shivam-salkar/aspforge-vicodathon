...
export const Aurora: React.FC<AuroraProps> = (props) => {
  const { colorStops = ['#3b82f6', '#8b5cf6', '#06b6d4'], amplitude = 1.0, blend = 0.5, speed = 0.5 } = props;
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      const currentAmp = propsRef.current.amplitude ?? amplitude;
      const currentBlend = propsRef.current.blend ?? blend;
      const timeVal = (propsRef.current.time ?? t * 0.01) * currentSpeed * 0.1;
  ...
  return <div ref={ctnDom} className={`aurora-container ${props.className || ''}`} style={props.style} />;
};
